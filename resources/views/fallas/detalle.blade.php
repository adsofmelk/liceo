		...
		<tbody>
			@foreach($fallas as $row)
				<tr>
					...
					<td>{!!link_to_route('fallas.edit',$title= "Modificar", $parameters = $row->idfallas, $attributes = ["class"=>'btn btn-primary'])!!}
						<div class='borrar btn btn-warning' data-id='{{$row->idfallas}}'>Borrar</div>
					</td>
				</tr>
			@endforeach
		</tbody>
